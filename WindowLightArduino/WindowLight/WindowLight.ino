...
void parseMessage(String message){
  ...
  if(message.length()>0){
    ...
    else if(annotation.equals("CL")){  
      setColor( data);
    }
    else if(annotation.equals("IC")){  
      setAlarmDuration( data);
    }
    else if(annotation.equals("ST")){   
      setTime( data);
    }
  }
}
...
void setTime(String data){
  ...
  int month = data.substring(0,2).toInt();
  Serial.print("month  \t");
  Serial.println(month);
  int day = data.substring(2,4).toInt();
  Serial.print("day  \t");
  Serial.println(day);

  int year = (data.substring(4,6).toInt())+2000;
  Serial.print("year  \t");
  Serial.println(year);
  int hour = data.substring(6,8).toInt();
  Serial.print("hour  \t");
  Serial.println(hour);
  int minutes= data.substring(8,10).toInt();
...
void setAlarm(String data){
  ...
  Serial.print("END Alarm Hour \t");
  Serial.println(endAlarmHour);
  Serial.print("END Alarm Minute \t");
  Serial.println(endAlarmMinute);


  tmElements_t alarmOnElements;
  alarmOnElements.Hour = alarmHour;
  alarmOnElements.Minute = alarmMinute;
  alarmOnElements.Second = 0;

  alarmOnElements.Day = day();
  alarmOnElements.Month = month();
  alarmOnElements.Year = year();

  tmElements_t alarmOffElements;
  alarmOffElements.Hour = endAlarmHour;
  alarmOffElements.Minute = endAlarmMinute;
  alarmOffElements.Second = 0;
  alarmOffElements.Day = day();
  alarmOffElements.Month = month();
  alarmOffElements.Year = year();
...
void setAlarmDuration(String data){
  ...
  Serial.println(alarmDurationMinutes);
